...
function ChatMessage({ sender, timestamp, message }) {
  return (
    <div className={`${sender === "AskNishauri" ? "justify-start" : "justify-end"} my-3 flex`}>
      <div className={`${sender === "AskNishauri" ? "bg-white text-black max-w-[75%]" : "bg-[#B273F0] text-white max-w-[75%]"} relative flex items-center  p-2 m-2 rounded-2xl`}>
        <div className="">
          {/* <div className={`hidden  min-w-[160px] font-bold ${sender === "AskNishauri" ? "text-[#29f6ac]" : "text-[#5aa2e0]" }`}>{sender}</div> */}
          <div className="absolute -bottom-4 right-6 text-black text-[10px]">{timestamp}</div>
        </div>
        <div className="min-w-[80px]">{message}</div>
